...
## Project Brief
A single-level 2D arcade shooter where the twist is the target: shoot the cute creatures, spare the monsters.
Parody-first, browser-based, pixel art aesthetic.

## Core Technologies
TypeScript, Phaser 3, esbuild

## Architecture
Static single-page app — one `index.html`, one compiled bundle:
- `GameScene` — the only Phaser scene; owns the game loop, spawn timer, and input handling
- `Creature` hierarchy — `Kawaii` (points on click) and `Monster` (health loss on click) extend a shared base
- `GameState` — plain object tracking score, health, and game-over flag
- Landscape drawn once per session with randomized variation; creatures emerge from behind it via z-order
